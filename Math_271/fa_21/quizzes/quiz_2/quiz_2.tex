...
\begin{problem}
For the following chemical reactions, determine an ODE that describes the rate of change of the concentration for a chemical species $A$.  Mention whether the equation is linear or nonlinear.
\begin{enumerate}[(a)]
    \item \textbf{(2 pts.)} $3A \to B$.
    \item \textbf{(2 pts.)} $B+C \to 2A$.
    \item \textbf{(BONUS 2 pts.)} $3A\to C+D \to 2E$.
\end{enumerate}
\end{problem}
