...
\setuptodonotes{size=\tiny,backgroundcolor=red!15!white} 
...
\title{Approaching Geometric Inverse Problems using Clifford Analysis (if you want to call it that now)}
...
\mycopyright{%
Copyright by Colin Roberts 2021 \\
All Rights Reserved
}
...
\abstract{
The geometrical inverse problem of determining an unknown Riemannian manifold $M$ from the Dirichlet-to-Neumann (DN) map on the boundary is known as the Calder\'on problem and, in dimension two, this problem is solvable up to conformal deformation. One technique is to use the DN map to recover the algebra of holomorphic functions on the manifold which, by Gelfand, this set is then homeomorphic to the manifold. In higher dimensions, one can replace the complex algebra bundle with differential forms, but another natural choice is to consider a geometric (or Clifford) algebra bundle whose sections are multivector fields. The graded algebra of multivector fields comes with a natural differential operator known as the gradient (or Dirac) operator that replaces the Hodge-Dirac operator in the exterior algebra of forms. Fields in the kernel of the gradient are monogenic, and these monogenic fields are a natural generalization of holomorphic functions. In this work, we prove a version of the Hodge-Morrey decomposition which splits the space of multivector fields into an orthogonal sum of monogenic fields and gradients. We define a suitable notion of a spectrum of the space of monogenic fields and prove the spectrum to be homeomorphic to the underlying manifold with the weak-$\ast$ topology.
}
...
\input{chapter_1/introduction.tex}
...
%potentially lay out what I will introduce in each chapter in a paragraph
\section{Geometric Spaces}
\label{subsec:quadratic_spaces}
\input{chapter_2/quadratic_spaces.tex}

\section{Clifford algebras}
\label{subsec:clifford_and_geometric_algebras}
\input{chapter_2/clifford_and_geometric_algebras.tex}

\section{Geometric algebras}

\section{Geometric manifolds}
\label{sec:geometric_manifolds}
\input{chapter_2/geometric_manifolds.tex}
...
\input{chapter_3/spaces_of_fields.tex}

\section{Algebras of fields}
\label{sec:algebras_of_fields}
\input{chapter_3/algebras_of_fields.tex}

\section{Gelfand theory}
\label{sec:gelfand_theory}
\input{chapter_3/gelfand_theory.tex}
...
\input{chapter_4/tomography.tex}

%\section{Relativistic Transport}
%\label{sec:relativistic_transport}
%\input{chapter_4/relativistic_transport}
